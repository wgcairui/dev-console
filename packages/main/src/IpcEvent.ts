...
class IpcEvent {
    ...
    private listen() {
        ...
        ipcMain.on("hexToNumber", (event, hex: string) => {
            const buf = Buffer.from(hex, 'hex')
            event.returnValue = buf.readIntBE(0, buf.length)
        })

        ipcMain.on("bufferToString", (event, hex: Buffer | ArrayBuffer | SharedArrayBuffer | any, encoding?: BufferEncoding) => {
            event.returnValue = Buffer.from(hex).toString(encoding || 'utf-8')
        })
