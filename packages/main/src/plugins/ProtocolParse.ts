...
class ProtocolParse {
    ...
    private parse232(data: Buffer, instructs: Uart.protocolInstruct) {
        ...
        return instructs.formResize.map<query>(el2 => {
            ...
            const regx = /^\(.*\)$/.test(el2.bl) ? this.ParseCoefficient(el2.bl, parseStr[start - 1] as any) : parseStr[start - 1]
            return { name: el2.name, regx: el2.bl, value: regx, parseValue: this.getUnit(regx as any, el2.unit!, el2.isState), unit: el2.unit }
        });
    }
    ...
    private parse485(data: Buffer, instructs: Uart.protocolInstruct) {
        ...
        return instructs.formResize.map(el2 => {
            // 申明结果
            const result: query = { name: el2.name, value: '0', unit: el2.unit, regx: '', parseValue: '' }
            // 每个数据的结果地址
